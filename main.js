document.addEventListener('DOMContentLoaded', function() {
  ...
  // ── DTR FLOATING CARD ──
  var dtrToggle  = document.getElementById('dtr-toggle');
  var dtrFloat   = document.getElementById('dtr-float');
  var dtrNext    = document.getElementById('dtr-next');
  var dtrPrev    = document.getElementById('dtr-prev');
  var dtrSlide1  = document.getElementById('dtr-slide-1');
  var dtrSlide2  = document.getElementById('dtr-slide-2');
  var dtrImgs    = document.querySelectorAll('.dtr-img');

  if (dtrToggle) {
    dtrToggle.addEventListener('click', function() {
      dtrFloat.classList.toggle('collapsed');
      dtrToggle.classList.toggle('collapsed');
    });
  }
  if (dtrNext) {
    dtrNext.addEventListener('click', function() {
      dtrSlide1.classList.remove('active');
      dtrSlide2.classList.add('active');
    });
  }
  if (dtrPrev) {
    dtrPrev.addEventListener('click', function() {
      dtrSlide2.classList.remove('active');
      dtrSlide1.classList.add('active');
    });
  }
  // DTR images open in lightbox (sharp)
  dtrImgs.forEach(function(img) {
    img.addEventListener('click', function() {
      lbImg.src = img.src;
      lbImg.alt = img.alt;
      lbImg.style.filter = 'none';  // sharp in lightbox
      lightbox.classList.add('active');
      document.body.style.overflow = 'hidden';
    });
  });

  // Reset lightbox filter on close
  var origCloseLB = closeLB;
  function closeLB() {
    lightbox.classList.remove('active');
    document.body.style.overflow = '';
    lbImg.style.filter = '';
